...
<div class="daily-check-unit bg-white rounded-xl shadow-sm p-4 sm:p-6 max-w-4xl mx-auto" id="wizardCekHarianUnit">
    ...
    @if(session('success'))
        <div class="mb-6 p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 flex items-center justify-between shadow-xs gap-3 flex-wrap">
            ...
            @if(session('cek_id'))
                <a href="{{ route('unit-rescue.cek-harian-unit-rescue.export-pdf', session('cek_id')) }}"
    ...
    <form action="{{ route('unit-rescue.cek-harian-unit-rescue.store') }}" method="POST" enctype="multipart/form-data" id="formCekHarianUnit" class="space-y-6">
        ...
        <div data-step-panel="3" class="hidden">
            <div class="mb-4 p-4 rounded-xl border border-blue-100 bg-blue-50/60"><h2 class="font-bold text-base text-gray-900">Pemeriksaan Kendaraan</h2><p class="text-xs text-gray-600 mt-1">Periksa setiap item satu per satu. Pilih kondisi dan tambahkan catatan bila diperlukan.</p></div>
