@extends('layouts.app')
@section('title', 'Riwayat Pengecekan — Pencegahan')
...
}" x-init="$watch('activeTab', () => $nextTick(() => { if (window.lucide) lucide.createIcons(); }))">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-5">
        <div>
            <h1 class="text-lg sm:text-2xl font-extrabold text-blue-950 flex items-center gap-2">
                <i data-lucide="history" class="w-5 h-5 sm:w-6 sm:h-6 text-emerald-600 flex-shrink-0"></i>
                <span>Riwayat Pengecekan Pencegahan</span>
            </h1>
            <p class="text-xs sm:text-sm text-gray-500 mt-0.5">Daftar rekapan hasil pemeriksaan harian unit armada dan peralatan pencegahan.</p>
        </div>
        <div class="grid grid-cols-2 gap-2 sm:flex sm:items-center">
            <a href="{{ route('unit-pencegahan.cek-harian-unit') }}"
               class="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition shadow-xs text-center">
    ...
    <div class="w-full sm:w-auto mb-4">
        <div class="grid grid-cols-2 sm:inline-flex items-center gap-1 p-1 bg-white border border-gray-200 rounded-lg shadow-2xs w-full sm:w-auto">
            <button type="button" @click="activeTab = 'unit'"
                    :class="activeTab === 'unit' ? 'bg-emerald-600 text-white shadow-xs' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'"
            ...
            <button type="button" @click="activeTab = 'alat'"
                    :class="activeTab === 'alat' ? 'bg-emerald-600 text-white shadow-xs' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'"
                    class="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 text-xs font-bold rounded-md transition whitespace-nowrap cursor-pointer">
                <i data-lucide="shield-alert" class="w-3.5 h-3.5"></i>
                <span>Peralatan Pencegahan</span>
