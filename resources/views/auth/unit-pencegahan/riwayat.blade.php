@extends('layouts.app')
@section('title', 'Riwayat Pengecekan dan Pengajuan — Pencegahan')
...
<div class="max-w-7xl mx-auto p-3.5 sm:p-6" x-data="{
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3.5 mb-5">
        <div class="space-y-1">
            <h1 class="text-xl sm:text-2xl font-black text-blue-950 flex items-center gap-2.5 tracking-tight">
                <span class="w-9 h-9 rounded-xl bg-emerald-100 text-emerald-600 flex items-center justify-center flex-shrink-0 shadow-2xs">
                    <i data-lucide="history" class="w-5 h-5"></i>
                </span>
                <span>Riwayat Pengecekan dan Pengajuan Pencegahan</span>
            </h1>
            <p class="text-xs sm:text-sm text-gray-500 max-w-2xl leading-relaxed">
                Daftar rekapan hasil pemeriksaan harian unit armada, peralatan, serta pengajuan pemeliharaan pencegahan.
            </p>
        </div>
    ...
    <div class="bg-white p-3.5 sm:p-4 rounded-xl border border-gray-200 shadow-2xs mb-4">
        <form method="GET" action="{{ route('unit-pencegahan.riwayat') }}" class="grid grid-cols-1 sm:grid-cols-12 gap-2.5 sm:gap-3 items-center">
            <input type="hidden" name="tab" :value="activeTab">
            
            <div class="sm:col-span-5 relative">
                <input type="text" name="search" value="{{ $searchQuery ?? '' }}"
                       placeholder="Cari Unit, Pemeriksa, Pos, Danru, Kabid..."
                       class="w-full pl-8 sm:pl-9 pr-3 py-2 text-xs sm:text-sm rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-emerald-600 bg-white">
                <i data-lucide="search" class="w-4 h-4 text-gray-400 absolute left-2.5 sm:left-3 top-1/2 -translate-y-1/2"></i>
            </div>

            <div class="sm:col-span-4 relative">
                <input type="date" name="tanggal" value="{{ $tanggal ?? '' }}"
                       class="w-full px-3 py-2 text-xs sm:text-sm rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-emerald-600 bg-white text-gray-700">
            </div>
            ...
            <div class="sm:col-span-3 flex items-center gap-2">
                ...
                @if(!empty($searchQuery) || !empty($tanggal))
                    <a href="{{ route('unit-pencegahan.riwayat') }}?tab={{ $tab }}"
    ...
    <div class="mb-4">
        <div class="bg-gray-100/90 p-1 rounded-xl border border-gray-200 inline-flex flex-nowrap overflow-x-auto no-scrollbar max-w-full gap-1 shadow-2xs">
            <button type="button" @click="activeTab = 'unit'"
                    :class="activeTab === 'unit' ? 'bg-white text-emerald-600 shadow-xs font-extrabold border border-gray-200/80' : 'text-gray-600 hover:text-gray-900 hover:bg-white/60 font-bold'"
                ...
                <span :class="activeTab === 'unit' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-200 text-gray-700'"
            ...
            <button type="button" @click="activeTab = 'alat'"
                    :class="activeTab === 'alat' ? 'bg-white text-emerald-600 shadow-xs font-extrabold border border-gray-200/80' : 'text-gray-600 hover:text-gray-900 hover:bg-white/60 font-bold'"
                    class="inline-flex items-center justify-center gap-2 px-3.5 sm:px-4 py-2 text-xs rounded-lg transition-all whitespace-nowrap flex-shrink-0 cursor-pointer">
                <i data-lucide="shield-alert" class="w-3.5 h-3.5"></i>
                <span>Peralatan<span class="hidden sm:inline"> Pencegahan</span></span>
                <span :class="activeTab === 'alat' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-200 text-gray-700'"
                      class="px-2 py-0.5 rounded-full text-[10px] font-extrabold transition">
                    {{ $cekAlatList->total() }}
                </span>
            </button>
            ...
            <button type="button" @click="activeTab = 'pengajuan'"
                    :class="activeTab === 'pengajuan' ? 'bg-white text-emerald-600 shadow-xs font-extrabold border border-gray-200/80' : 'text-gray-600 hover:text-gray-900 hover:bg-white/60 font-bold'"
                    class="inline-flex items-center justify-center gap-2 px-3.5 sm:px-4 py-2 text-xs rounded-lg transition-all whitespace-nowrap flex-shrink-0 cursor-pointer">
                <i data-lucide="clipboard-list" class="w-3.5 h-3.5"></i>
                <span>Pengajuan</span>
                <span :class="activeTab === 'pengajuan' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-200 text-gray-700'"
                      class="px-2 py-0.5 rounded-full text-[10px] font-extrabold transition">
                    {{ $pengajuanList->total() }}
                </span>
            </button>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-2xs overflow-hidden">
            @if($cekUnitList->isEmpty())
                <div class="p-10 sm:p-12 text-center">
                    ...
                    <h3 class="text-sm font-bold text-gray-800">Belum Ada Riwayat Pengecekan Unit Pencegahan</h3>
                    <p class="text-xs text-gray-500 mt-1">Data pengecekan unit pencegahan yang disimpan akan muncul di sini.</p>
                </div>
            @else
                            ...
                            <tr class="hover:bg-emerald-50/20 transition">
                                ...
                                <td class="py-3 px-3.5 text-center whitespace-nowrap">
                                    <div class="inline-flex items-center gap-1.5">
                                        <a href="{{ route('unit-pencegahan.cek-harian-unit.export-pdf', $item->id) }}"
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-2xs overflow-hidden">
            @if($cekAlatList->isEmpty())
                <div class="p-10 sm:p-12 text-center">
                    ...
                    <h3 class="text-sm font-bold text-gray-800">Belum Ada Riwayat Pengecekan Alat Pencegahan</h3>
                    <p class="text-xs text-gray-500 mt-1">Data pengecekan alat pencegahan yang disimpan akan muncul di sini.</p>
                </div>
            @else
                {{-- DESKTOP & TABLET TABLE VIEW (Hidden on Mobile) --}}
                <div class="hidden md:block overflow-x-auto">
                    <table class="w-full text-left text-xs border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-emerald-50/20 transition">
                                ...
                                <td class="py-3 px-3.5 text-center whitespace-nowrap">
                                    <div class="inline-flex items-center gap-1.5">
                                        <a href="{{ route('alat-pencegahan.cek-harian-alat.export-pdf', $item->id) }}"
                                           title="Unduh PDF Resmi"
                                           class="inline-flex items-center gap-1 px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md text-[11px] font-bold transition shadow-2xs">
                                            <i data-lucide="download" class="w-3.5 h-3.5"></i>
                                            <span>PDF</span>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-2xs overflow-hidden">
            @if($pengajuanList->isEmpty())
                <div class="p-10 sm:p-12 text-center">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Data surat permohonan pemeliharaan atau perbaikan pencegahan yang diajukan akan tercatat di sini.</p>
                    <a href="{{ route('pemeliharaan.pengajuan') }}"
                       class="inline-flex items-center gap-1.5 px-3.5 py-2 mt-4 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition shadow-xs">
            ...
            <template x-if="selectedUnit">
                <div class="flex flex-col h-full min-h-0">
                    ...
                    <div class="flex-shrink-0 p-4 sm:p-6 border-b border-gray-200">
                        ...
                        <p class="text-xs text-gray-500 mb-3.5" x-text="'Tanggal: ' + formatDate(selectedUnit.created_at || selectedUnit.tanggal_pemeriksaan) + ' | Pos: ' + (selectedUnit.pos || '-')"></p>
                    ...
                    <div class="flex-shrink-0 flex flex-col-reverse sm:flex-row justify-end gap-2 p-4 sm:p-6 border-t border-gray-200">
                        ...
                        <a :href="'/unit-pencegahan/cek-harian-unit/' + selectedUnit.id + '/export-pdf'"
            ...
            <template x-if="selectedAlat">
                <div class="flex flex-col h-full min-h-0">
                    ...
                    <div class="flex-shrink-0 flex flex-col-reverse sm:flex-row justify-end gap-2 p-4 sm:p-6 border-t border-gray-200">
                        ...
                        <a :href="'/alat-pencegahan/cek-harian-alat/' + selectedAlat.id + '/export-pdf'"
                           class="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold transition shadow-xs text-center">
                            <i data-lucide="download" class="w-4 h-4"></i>
                            <span>Unduh PDF</span>
                        </a>
                ...
                <div class="flex flex-col h-full min-h-0">
                    ...
                    <div class="flex-1 min-h-0 overflow-y-auto p-4 sm:p-6 space-y-4">
                        ...
                        <div x-show="selectedPengajuan.status === 'disetujui'"
                             class="p-3 bg-emerald-50/70 border border-emerald-200 rounded-xl text-xs space-y-1">
