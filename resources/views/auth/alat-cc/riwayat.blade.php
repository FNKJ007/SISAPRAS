@extends('layouts.app')
@section('title', 'Riwayat Pengecekan dan Pengajuan — Command Center')
...
<div class="max-w-7xl mx-auto p-3.5 sm:p-6" x-data="{
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3.5 mb-5">
        <div class="space-y-1">
            <h1 class="text-xl sm:text-2xl font-black text-blue-950 flex items-center gap-2.5 tracking-tight">
                <span class="w-9 h-9 rounded-xl bg-blue-100 text-blue-800 flex items-center justify-center flex-shrink-0 shadow-2xs">
                    <i data-lucide="history" class="w-5 h-5"></i>
                </span>
                <span>Riwayat Pengecekan dan Pengajuan Command Center</span>
            </h1>
            <p class="text-xs sm:text-sm text-gray-500 max-w-2xl leading-relaxed">
                Daftar rekapan hasil pemeriksaan harian peralatan, fasilitas, serta pengajuan pemeliharaan Command Center.
            </p>
        </div>
        ...
        <form method="GET" action="{{ route('alat-cc.riwayat') }}" class="grid grid-cols-1 sm:grid-cols-12 gap-2.5 sm:gap-3 items-center">
            ...
            <div class="sm:col-span-5 relative">
                <input type="text" name="search" value="{{ $searchQuery ?? '' }}"
                       placeholder="Cari Pemeriksa, Pos, Danru, Item, No. Lambung..."
            ...
            <div class="sm:col-span-3 flex items-center gap-2">
                ...
                @if(!empty($searchQuery) || !empty($tanggal))
                    <a href="{{ route('alat-cc.riwayat', ['tab' => $tab ?? 'alat']) }}"
    ...
    {{-- Tabs Switcher (Peralatan vs Pengajuan) --}}
    <div class="mb-4">
        <div class="bg-gray-100/90 p-1 rounded-xl border border-gray-200 inline-flex flex-nowrap overflow-x-auto no-scrollbar max-w-full gap-1 shadow-2xs">
            <button type="button" @click="activeTab = 'alat'"
                    :class="activeTab === 'alat' ? 'bg-white text-blue-900 shadow-xs font-extrabold border border-gray-200/80' : 'text-gray-600 hover:text-gray-900 hover:bg-white/60 font-bold'"
                ...
                <span>Peralatan<span class="hidden sm:inline"> Command Center</span></span>
                <span :class="activeTab === 'alat' ? 'bg-blue-100 text-blue-800' : 'bg-gray-200 text-gray-700'"
            ...
            <button type="button" @click="activeTab = 'pengajuan'"
                    :class="activeTab === 'pengajuan' ? 'bg-white text-blue-900 shadow-xs font-extrabold border border-gray-200/80' : 'text-gray-600 hover:text-gray-900 hover:bg-white/60 font-bold'"
                    class="inline-flex items-center justify-center gap-2 px-3.5 sm:px-4 py-2 text-xs rounded-lg transition-all whitespace-nowrap flex-shrink-0 cursor-pointer">
                <i data-lucide="clipboard-list" class="w-3.5 h-3.5"></i>
                <span>Pengajuan</span>
                <span :class="activeTab === 'pengajuan' ? 'bg-blue-100 text-blue-800' : 'bg-gray-200 text-gray-700'"
                      class="px-2 py-0.5 rounded-full text-[10px] font-extrabold transition">
                    {{ $pengajuanList->total() }}
                </span>
            </button>
    ...
    {{-- ===================== TAB 1: TABEL & MOBILE CARDS PERALATAN ===================== --}}
    <div x-show="activeTab === 'alat'" x-cloak class="bg-white rounded-xl border border-gray-200 shadow-2xs overflow-hidden">
        ...
        @else
            ...
            <div class="p-3 sm:p-3.5 border-t border-gray-100 bg-gray-50/50">
                {{ $cekAlatList->appends(array_merge(request()->query(), ['tab' => 'alat']))->links('vendor.pagination.custom') }}
            </div>
        @endif
    </div>

    {{-- ===================== TAB 2: TABEL & MOBILE CARDS PENGAJUAN ===================== --}}
    <div x-show="activeTab === 'pengajuan'" x-cloak class="bg-white rounded-xl border border-gray-200 shadow-2xs overflow-hidden">
        @if($pengajuanList->isEmpty())
            <div class="p-10 sm:p-12 text-center">
                <div class="w-12 h-12 rounded-full bg-gray-50 border border-gray-200 flex items-center justify-center mx-auto mb-3 text-gray-400">
                    <i data-lucide="file-text" class="w-6 h-6"></i>
                </div>
                <h3 class="text-sm font-bold text-gray-800">Belum Ada Riwayat Pengajuan</h3>
                <p class="text-xs text-gray-500 mt-1">Form permohonan pemeliharaan yang telah Anda ajukan akan muncul di sini.</p>
                <div class="mt-4">
                    <a href="{{ route('pemeliharaan.pengajuan') }}"
                       class="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-900 hover:bg-blue-950 text-white rounded-lg text-xs font-bold transition shadow-xs">
        ...
        @else
            ...
            <div class="hidden md:block overflow-x-auto">
                <table class="w-full text-left text-xs border-collapse">
                    <thead>
                        <tr class="bg-gray-50/80 border-b border-gray-200 text-gray-600 font-bold uppercase tracking-wider text-[11px]">
                            <th class="py-3 px-3.5 text-center w-12">No</th>
                            <th class="py-3 px-3.5">Kode / Tgl</th>
                            <th class="py-3 px-3.5">Nomor Lambung / Unit</th>
                            <th class="py-3 px-3.5">Pemegang & Regu</th>
                            <th class="py-3 px-3.5">Rincian Perbaikan</th>
                            <th class="py-3 px-3.5 text-center">Status Verifikasi</th>
                            <th class="py-3 px-3.5 text-center">Status Bengkel</th>
                            <th class="py-3 px-3.5 text-center w-36">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-blue-50/20 transition">
                            ...
                            <td class="py-3 px-3.5 whitespace-nowrap">
                                <span class="font-mono text-[11px] font-bold text-gray-500 block">
                                    {{ $item->kode_verifikasi }}
                                </span>
                                <span class="text-gray-900 font-semibold text-xs block">
                            ...
                            <td class="py-3 px-3.5">
                                <span class="font-extrabold text-blue-950 block text-xs">
                                    {{ $item->nomor_lambung ?? '-' }}
                                </span>
                            ...
                            <td class="py-3 px-3.5">
                                ...
                                <span class="text-[10px] text-gray-500 block">{{ $item->nip_pemegang ?? 'Tanpa NIP' }}</span>
                                <span class="text-[10px] text-blue-800 font-semibold block">{{ $item->regu ?? 'Regu 1' }}</span>
                            </td>
                            <td class="py-3 px-3.5 max-w-xs">
                                @if($item->verified_item_list && count($item->verified_item_list) > 0)
                                    <div class="flex flex-wrap gap-1">
                                        @foreach(array_slice($item->verified_item_list, 0, 3) as $it)
                                            <span class="inline-block px-1.5 py-0.5 rounded text-[10px] bg-gray-100 text-gray-700 border border-gray-200">
                                                {{ $it }}
                                            </span>
                                        @endforeach
                                        @if(count($item->verified_item_list) > 3)
                                            <span class="text-[10px] text-gray-400">+{{ count($item->verified_item_list) - 3 }} lainnya</span>
                                        @endif
                                    </div>
                                @else
                                    <p class="text-gray-700 truncate" title="{{ $item->item_perbaikan }}">
                                        {{ $item->item_perbaikan ?? '-' }}
                                    </p>
                                @endif
                            </td>
                            ...
                            <td class="py-3 px-3.5 text-center whitespace-nowrap">
                                ...
                                @elseif($item->status_pengerjaan === 'proses' || (!empty($item->tanggal_keberangkatan) && $item->status_pengerjaan !== 'selesai'))
            ...
            <div class="block md:hidden divide-y divide-gray-100">
                ...
                <div class="p-3.5 space-y-2.5">
                    <div class="flex items-start justify-between gap-2">
                        <div>
                            <span class="text-[10px] font-mono text-gray-400 font-bold block">{{ $item->kode_verifikasi }}</span>
                            <h3 class="font-extrabold text-xs text-blue-950 leading-snug">{{ $item->nomor_lambung ?? 'Pengajuan Unit' }}</h3>
                            <span class="text-[10px] text-gray-500 font-medium">{{ $item->jenis_kendaraan ?? '-' }} • {{ $item->pos ?? '-' }}</span>
                        </div>
                        <div class="flex flex-col items-end gap-1 flex-shrink-0">
                            @if($item->status === 'disetujui')
                            ...
                            @if($item->status_pengerjaan === 'selesai')
                                <span class="text-[9px] font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.2 rounded border border-emerald-200">Bengkel: Selesai</span>
                            @elseif($item->status_pengerjaan === 'proses' || (!empty($item->tanggal_keberangkatan) && $item->status_pengerjaan !== 'selesai'))
                                <span class="text-[9px] font-bold text-orange-700 bg-orange-50 px-1.5 py-0.2 rounded border border-orange-200">Bengkel: Dikerjakan</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 text-xs bg-gray-50 p-2.5 rounded-lg border border-gray-100">
                        <div>
                            <span class="text-[10px] font-semibold text-gray-400 block uppercase">Tanggal</span>
                            <span class="font-bold text-gray-800 text-[11px] block">{{ $item->created_at->translatedFormat('d M Y') }}</span>
                            <span class="text-[10px] text-gray-400">{{ $item->created_at->format('H:i') }} WIB</span>
                        </div>
                        <div>
                            <span class="text-[10px] font-semibold text-gray-400 block uppercase">Pemegang</span>
                            <span class="font-bold text-gray-800 text-[11px] block truncate">{{ $item->nama_pemegang }}</span>
                            <span class="text-[10px] text-gray-400 block truncate">{{ $item->nip_pemegang ?? '-' }}</span>
                        </div>
                        <div class="col-span-2">
                            <span class="text-[10px] font-semibold text-gray-400 block uppercase">Item Perbaikan</span>
                            <span class="text-[11px] text-gray-700 font-medium block truncate">{{ $item->item_perbaikan ?? '-' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 pt-1">
                        <a href="{{ route('pemeliharaan.pengajuan.cetak-dokumen', ['id' => $item->id, 'type' => 'permohonanbidang']) }}"
                           target="_blank"
                           class="inline-flex items-center justify-center gap-1 px-3 py-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white rounded-lg text-xs font-bold transition shadow-xs text-center">
                            <i data-lucide="printer" class="w-3.5 h-3.5"></i>
            ...
            {{-- Pagination --}}
            <div class="p-3 sm:p-3.5 border-t border-gray-100 bg-gray-50/50">
                {{ $pengajuanList->appends(array_merge(request()->query(), ['tab' => 'pengajuan']))->links('vendor.pagination.custom') }}
            </div>
        @endif
    </div>
