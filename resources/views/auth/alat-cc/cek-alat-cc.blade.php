...
<div class="bg-white rounded-xl shadow-sm p-4 sm:p-6 max-w-5xl mx-auto">
    ...
    @if(session('success'))
        <div class="mb-6 p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 flex items-center justify-between shadow-xs">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h4 class="font-bold text-sm">Pemeriksaan Alat Command Center Berhasil Disimpan!</h4>
    ...
    <p class="text-gray-500 text-sm mt-1 mb-6">
        Pemeriksaan kondisi dan kelengkapan peralatan dan fasilitas Command Center.
    </p>

    <form action="{{ route('alat-cc.cek-alat-cc.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf

        {{-- Identitas Pemeriksaan: Regu, Nama Pemeriksa, Jabatan, Tanggal --}}
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
                <label for="pos" class="block text-sm font-medium mb-1">Regu <span class="text-red-500">*</span></label>
        ...
        {{-- Daftar Pemeriksaan Alat Command Center --}}
        <div class="pt-2">
            <h2 class="text-base font-semibold border-b border-gray-200 pb-2 mb-4">Daftar Pemeriksaan Alat Command Center</h2>
                    ...
                    <div class="border border-gray-200 rounded-xl p-4 sm:p-5"
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
                            <div class="flex items-center gap-3 flex-1">
                                <span class="flex-shrink-0 w-8 h-8 rounded-md bg-blue-900 text-white text-xs font-bold flex items-center justify-center">
                            ...
                            <div class="flex items-center gap-3 sm:gap-4 flex-wrap">
                                <div class="flex items-center gap-2">
                                    <label for="baik_{{ $index }}" class="text-xs font-bold text-emerald-700 uppercase tracking-wide">Baik:</label>
                                    <input type="number" id="baik_{{ $index }}" name="alat[{{ $index }}][jumlah_baik]"
                                           x-model.number="jumlahBaik" min="0" placeholder="0"
                                           class="w-20 rounded-lg border border-emerald-300 px-3 py-1.5 text-sm font-semibold text-emerald-800 bg-emerald-50/60 focus:outline-none focus:ring-2 focus:ring-emerald-600">
                                </div>
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-100" x-show="jumlahRusak > 0" x-cloak>
                            ...
                            <input type="text" name="alat[{{ $index }}][nomor_rusak]"
                                   ...
                                   placeholder="Contoh: Unit 1 mengalami gangguan daya"
        ...
        <div class="flex justify-end pt-4 border-t border-gray-200">
            <button type="submit" class="btn btn-primary">
                <i data-lucide="send" class="w-4 h-4"></i> Simpan Pemeriksaan Command Center
            </button>
        </div>
