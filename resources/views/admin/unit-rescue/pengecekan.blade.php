...
@section('title', 'Pengecekan Unit Rescue — Admin')

@section('content')
<div class="admin-pengecekan-wrapper" x-data="pengecekanRescueAdmin('{{ $tab }}')">

    {{-- Header Page --}}
    <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px; margin-bottom:20px;">
        <div>
            <h1 style="font-size:22px; font-weight:800; color:#121E4E; margin:0 0 2px 0;">Pengecekan Unit Rescue</h1>
            <p style="font-size:13px; color:#64748B; margin:0;">Hasil pengecekan harian unit kendaraan dan alat rescue yang diinput oleh petugas.</p>
        </div>
    ...
    <div style="background:#FFFFFF; border-radius:14px; padding:12px 16px; margin-bottom:18px; border:1px solid #E2E8F0; box-shadow:0 2px 8px rgba(0,0,0,0.03); display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px;">
        ...
        <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
            <form method="GET" action="{{ route('admin.unit-rescue.pengecekan') }}" style="display:flex; align-items:center; gap:8px;">
                ...
                @if(!empty($searchQuery))
                    <a :href="'{{ route('admin.unit-rescue.pengecekan') }}?tab=' + activeTab" style="padding:8px 12px; background:#F1F5F9; color:#64748B; border:1px solid #CBD5E1; border-radius:10px; font-size:12px; font-weight:600; text-decoration:none;">
                        Reset
                    </a>
                @endif
            </form>
            ...
            <a x-cloak x-show="activeTab === 'unit'"
               href="{{ route('admin.unit-rescue.pengecekan.export-excel-unit', ['search' => $searchQuery ?? '']) }}"
               ...
               title="Download Rekap Pengecekan Unit Rescue ke Excel (.xlsx)">
            ...
            <a x-cloak x-show="activeTab === 'alat'"
               href="{{ route('admin.unit-rescue.pengecekan.export-excel-alat', ['search' => $searchQuery ?? '']) }}"
               class="inline-flex flex-row items-center justify-center gap-2 whitespace-nowrap shrink-0"
               style="display:inline-flex !important; flex-direction:row !important; align-items:center !important; justify-content:center !important; gap:8px !important; white-space:nowrap !important; flex-shrink:0 !important; padding:9px 16px; background:#10B981; color:#FFFFFF; border:none; border-radius:10px; font-size:12.5px; font-weight:700; text-decoration:none; box-shadow:0 3px 8px rgba(16,185,129,0.25); transition:all 0.2s ease;"
               title="Download Rekap Pengecekan Alat Rescue ke Excel (.xlsx)">
                <i data-lucide="file-spreadsheet" style="width:16px; height:16px; min-width:16px; min-height:16px; flex-shrink:0; display:inline-block; vertical-align:middle;"></i>
                ...
                <div style="padding:56px 20px; text-align:center; background:#FFFFFF; display:flex; flex-direction:column; align-items:center; justify-content:center;">
                    ...
                    @if(!empty($searchQuery))
                        ...
                        <div style="font-size:13px; color:#64748B; margin-bottom:18px; max-width:440px; margin-left:auto; margin-right:auto;">
                            Tidak ditemukan data pengecekan unit rescue yang cocok dengan kata kunci <strong style="color:#1E293B;">"{{ $searchQuery }}"</strong>.
                        </div>
                        <a href="{{ route('admin.unit-rescue.pengecekan') }}?tab=unit" style="display:inline-flex; align-items:center; gap:8px; padding:9px 20px; background:#1B2A6B; color:#FFFFFF; border-radius:10px; font-size:12.5px; font-weight:700; text-decoration:none; box-shadow:0 4px 12px rgba(27,42,107,0.2);">
                            <i data-lucide="rotate-ccw" style="width:14px; height:14px;"></i>
                            <span>Reset Pencarian</span>
                        </a>
                    @else
                        <div style="font-size:16px; font-weight:800; color:#0F172A; margin-bottom:6px;">Belum Ada Data Pengecekan</div>
                        <div style="font-size:12.5px; color:#94A3B8;">Belum ada petugas yang mengisi Cek Harian Unit Kendaraan Rescue.</div>
                    @endif
                </div>
            @else
            ...
            @if($cekAlatList->isEmpty())
                <div style="padding:56px 20px; text-align:center; background:#FFFFFF; display:flex; flex-direction:column; align-items:center; justify-content:center;">
                    ...
                    @if(!empty($searchQuery))
                        <div style="font-size:16px; font-weight:800; color:#0F172A; margin-bottom:6px;">Data Tidak Ditemukan</div>
                        <div style="font-size:13px; color:#64748B; margin-bottom:18px; max-width:440px; margin-left:auto; margin-right:auto;">
                            Tidak ditemukan data pengecekan alat rescue yang cocok dengan kata kunci <strong style="color:#1E293B;">"{{ $searchQuery }}"</strong>.
                        </div>
                        <a href="{{ route('admin.unit-rescue.pengecekan') }}?tab=alat" style="display:inline-flex; align-items:center; gap:8px; padding:9px 20px; background:#1B2A6B; color:#FFFFFF; border-radius:10px; font-size:12.5px; font-weight:700; text-decoration:none; box-shadow:0 4px 12px rgba(27,42,107,0.2);">
                            <i data-lucide="rotate-ccw" style="width:14px; height:14px;"></i>
                            <span>Reset Pencarian</span>
                        </a>
                    @else
                        <div style="font-size:16px; font-weight:800; color:#0F172A; margin-bottom:6px;">Belum Ada Data Pengecekan</div>
                        <div style="font-size:12.5px; color:#94A3B8;">Belum ada petugas yang mengisi Cek Harian Alat Rescue.</div>
                    @endif
                </div>
            @else
                <div style="overflow-x:auto; -webkit-overflow-scrolling:touch;">
    ...
    {{-- ===================== MODAL DETAIL: UNIT KENDARAAN RESCUE ===================== --}}
    <div x-show="unitModalOpen" x-cloak class="admin-modal-overlay"
         style="position:fixed; top:0; left:0; width:100vw; height:100vh; z-index:99999; display:flex; align-items:center; justify-content:center; padding:16px; background-color:rgba(15,23,42,0.75);">
...
@push('scripts')
<script>
function pengecekanRescueAdmin(initialTab) {
    return {
