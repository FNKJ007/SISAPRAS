...
<div>
    ...
    <div class="no-print" style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px; flex-wrap:wrap; gap:12px;">
        <div>
            ...
            <p style="font-size:13px; color:#64748B; margin-top:4px; margin-bottom:0;">
                Ledger saldo kumulatif pemeliharaan berdasarkan data Monitoring Aktual, per tahun anggaran.
            </p>
        </div>
    ...
    <div class="kpi-grid-container no-print" style="margin-bottom:24px;">
        <div style="background:#FFFFFF; padding:18px; border-radius:14px; border:1px solid #E2E8F0; box-shadow:0 2px 8px rgba(15,23,42,0.04);">
            <div style="color:#64748B; font-size:11.5px; font-weight:700; text-transform:uppercase;">Total Record (Filter Aktif)</div>
        ...
        <div style="background:#FFFFFF; padding:18px; border-radius:14px; border:1px solid #E2E8F0; box-shadow:0 2px 8px rgba(15,23,42,0.04);">
            <div style="color:#059669; font-size:11.5px; font-weight:700; text-transform:uppercase;">Sudah Lunas / Selesai ({{ $kpi['jumlah_lunas'] }})</div>
            <div style="font-size:18px; font-weight:800; color:#059669; margin-top:6px;">Rp {{ number_format($kpi['total_lunas'], 0, ',', '.') }}</div>
        </div>
        <div style="background:#FFFFFF; padding:18px; border-radius:14px; border:1px solid #E2E8F0; box-shadow:0 2px 8px rgba(15,23,42,0.04);">
            <div style="color:#D97706; font-size:11.5px; font-weight:700; text-transform:uppercase;">Proses / Belum ({{ $kpi['jumlah_belum'] }})</div>
        ...
        <div id="kartu-kendali-print-area" style="padding:24px 28px; background:#FFFFFF; max-width:794px; margin:0 auto; box-sizing:border-box;">
            ...
            @if($kartuKendaliRows->isEmpty())
                <div style="padding:56px 20px; text-align:center; background:#FFFFFF; display:flex; flex-direction:column; align-items:center; justify-content:center;">
                    ...
                    <div style="font-size:16px; font-weight:800; color:#0F172A; margin-bottom:6px;">Belum Ada Data Monitoring Aktual</div>
                    <div style="font-size:13px; color:#64748B; max-width:400px; margin:0 auto;">Tidak ditemukan data monitoring aktual untuk tahun anggaran / filter yang dipilih.</div>
                </div>
            @else
                {{-- Tabel Presisi A4 --}}
                    ...
                    <table style="width:100%; border-collapse:collapse; font-size:11px; text-align:left; table-layout:auto;">
                        <thead>
                            <tr style="background:#1B2A6B; color:#FFFFFF; font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:0.5px;">
                                ...
                                <th style="padding:8px 8px; width:150px;">Nomor Invoice / Rujukan</th>
